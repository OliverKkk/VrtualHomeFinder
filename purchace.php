<?php 
	...
	header("location:details.php?error={$error}&id={$id}&name={$name}&number={$encNo}&email={$email}&pobox={$address}#frm");
		exit;
...
<table width="100%" border="0" cellspacing="0" cellpadding="3">
 ...
 <tr>
	...
	&nbsp;<br />
	 ...
	 <?php 
		...
		$result=getResult("select d.description,d.rooms,d.water_connection,
	                     d.elec_connection,h.price,h.house_id
	                     from house_details d , houses h  
						 where d.house_id={$id} and h.house_id={$id}");
		
		
		$to = getPurchaceMail();
		$subject = ' New Purchace request ';	   
        $headers = "From: www.pointtopoint.com\r\nX-Mailer: Custom PHP Script"."\n";
	   
	   $info="\n The client has selected the house with the following details: \n
			     *************************************************************\n";
			...
			foreach($row as $key=>$current){
				...
				$title=mysql_field_name($result,$index);
	...
	<table border="0" cellspacing="0" cellpadding="0" width="190">
			...
			<?php 
				$result=getResult("select house_id from houses limit 5,8");
